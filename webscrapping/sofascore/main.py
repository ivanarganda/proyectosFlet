...
from Services.scrapper import RenderScrapper
...
def route_change(e: ft.RouteChangeEvent):
    ...
    try:
        ...
        # === TASKS ===
        elif route == "/services/scrapper":
            page.views.clear()
            if not is_logged_in:
                page.views.append(ft.View("/services/scrapper", controls=[ft.Text("")]))
                show_session_expired_dialog(page)
            else:
                page.views.append(ft.View("/services/scrapper", controls=[RenderScrapper(page)]))
            page.update()
        # === 404 ===
        else:
