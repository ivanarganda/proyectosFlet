...
# 4. CARGAR ESCUDOS
# ==========================================================
df_escudos = read_file(f"{INITTED_FOLDER}escudos_equipo.xlsx")
...
jornadas = init_excel_db(
    ...
    ["id_jornada", "id_temporada"],
    [df_jorn["id_jornada"].tolist(), df_jorn["id_temporada"].tolist()]
)
...
temporadas = init_excel_db(
    ...
    ["id_temporada", "nombre", "year_start", "year_end"],
    [df_jorn["id_temporada"].tolist(), nombres_temp, years_start, years_end]
)
...
output_path = f"{INITTED_FOLDER}jugadores_stats_db.xlsx"
...
df_stats = pd.DataFrame(pd.read_excel(f"{INITTED_FOLDER}jugadores_stats_db.xlsx"))
...
posiciones = dict(
    zip( names_posiciones, ids_posiciones )
)
